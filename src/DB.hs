  ...
  where
    ...
    sqQuery =
      [SQQQ.sql|
            ...
            from metric_data
                    join (select id, json_object(name, description, low_outcome, low_outcome_text, high_outcome,high_outcome_text, json(bins) as bins, unit from metrics group by id) as m on metric_data.metric_id = m.id
